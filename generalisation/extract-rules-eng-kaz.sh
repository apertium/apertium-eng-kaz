...
DEFINE_string 'corpus' '/home/apertium/apertium-testing/apertium-eng-kaz/texts/texts' 'prefix of files containing parallel corpus (suffixes are .SL and .TL)' 'c'
...
######################
DEFINE_string 'tmp_dir' '/home/apertium/apertium-testing/apertium-eng-kazNEW' 'temporary directory' 'm'
...
DEFINE_string 'test_corpus' '/home/apertium/apertium-testing/apertium-eng-kazNEW/texts/' 'evaluation corpus prefix (suffixes are .SL and .TL)' 'e'
DEFINE_string 'dev_corpus' 'home/apertium/apertium-testing/apertium-eng-kazNEW/dev-texts/' 'development corpus prefix (suffixes are .SL and .TL). It is used to choose the most appropriate threshold' 'v'
...
DEFINE_string 'corpus_head_size' '35' 'size (in lines) of the prefix of the corpus to use' 'a'
...
if [ -f $DATADIR/$SL.dix ] ; then
	MONODIXL1=$DATADIR/$SL.dix
echo MONODIXL1
...
#find monolingual .dix files
if [ -f /$TL.lexc ] ; then
	MONODIXL2=$DATADIR2/$TL.lexc
elif [ -f $DATADIR2/apertium-${TL}.$TL.lexc ]; then
	MONODIXL2=$DATADIR2/apertium-${TL}.$TL.lexc
elif [ -f $DATADIR/apertium-${TL}.$TL.dixtmp1 ]; then
	MONODIXL2=$DATADIR2/apertium-${TL}.$TL.dixtmp1
else
...
if [ "${FLAGS_transfer_tools_1}" == "${FLAGS_TRUE}" ]; then
  ...
  if [ "${FLAGS_only_extract_bilingual_phrases}" != "${FLAGS_TRUE}" ]; then
    ...
    APERTIUMPREFIXDATADIR="$APERTIUMPREFIX/"
