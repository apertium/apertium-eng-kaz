...
PARDEF_FILES=/home/aida/apertium-testing/apertium-kaz/tests/morphotactics/kaz-eng/*.txt.gz

echo "==Kazakh->English==========================="

echo "" > $TMPDIR/kaz-eng.testvoc

for file in $PARDEF_FILES; do
    zcat $file | extract_lexical_units |
    $INCONSISTENCY kaz-eng >> $TMPDIR/kaz-eng.testvoc
done

$INCONSISTENCY_SUMMARY $TMPDIR/kaz-eng.testvoc kaz-eng
