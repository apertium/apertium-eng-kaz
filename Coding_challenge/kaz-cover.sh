...
cat /home/apertium/apertium-testing/apertium-eng-kaz/Coding_challenge/corpus\ Lab\ IIS\ \(5925\).kz.txt  | cut -f2 | grep -v '>(' | sed 's/&lt;/</g' | sed 's/&gt;/>/g' | apertium-destxt | lt-proc -w $BIN | apertium-retxt | sed 's/\$\W*\^/$\n^/g' > /tmp/kaz.coverage.txt
